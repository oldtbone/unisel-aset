        ...
        <div class="col-md-12">
            @can('asset_model_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.asset-models.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $assetModel->id }}">
                                        ...
                                        <td>
                                            @can('asset_model_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.asset-models.show', $assetModel->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('asset_model_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.asset-models.edit', $assetModel->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('asset_model_delete')
                                                <form action="{{ route('frontend.asset-models.destroy', $assetModel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('asset_model_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.asset-models.massDestroy') }}",
