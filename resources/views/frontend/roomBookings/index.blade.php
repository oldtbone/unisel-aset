        ...
        <div class="col-md-12">
            @can('room_booking_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.room-bookings.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $roomBooking->id }}">
                                        ...
                                        <td>
                                            @can('room_booking_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.room-bookings.show', $roomBooking->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('room_booking_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.room-bookings.edit', $roomBooking->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('room_booking_delete')
                                                <form action="{{ route('frontend.room-bookings.destroy', $roomBooking->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('room_booking_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.room-bookings.massDestroy') }}",
