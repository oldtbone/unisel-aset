                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.faculties.update", [$faculty->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="code" id="code" value="{{ old('code', $faculty->code) }}" required>
                            @if($errors->has('code'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="status" id="status" required>
                                ...
                                @foreach(App\Models\Faculty::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $faculty->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
