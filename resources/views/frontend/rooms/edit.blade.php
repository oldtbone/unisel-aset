                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.rooms.update", [$room->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $room->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="jpa_code" id="jpa_code" value="{{ old('jpa_code', $room->jpa_code) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="capacity" id="capacity" value="{{ old('capacity', $room->capacity) }}" step="1" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="room_type_id" id="room_type_id" required>
                                @foreach($room_types as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('room_type_id') ? old('room_type_id') : $room->room_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="location_id" id="location_id" required>
                                @foreach($locations as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('location_id') ? old('location_id') : $room->location->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="note" id="note">{{ old('note', $room->note) }}</textarea>
                            @if($errors->has('note'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('note') }}
                                </div>
